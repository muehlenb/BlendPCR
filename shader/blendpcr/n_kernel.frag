...
uniform sampler2D texture2D_aKernel;
...
void calc_eigenvalues_unopt(in mat3 m, in out vec3 lambda )
{
	...
	float c = m[0][0]*(h2-h1) + h3*m[2][2] - 2*m[0][1]*m[0][2]*m[1][2] +
			   h4*m[1][1];
...
int calc_eigenvector(mat3 m, in float lambda, out vec3 v )
{
	...
	float vv[EIG_DIM];
	vv[EIG_DIM-1] = 1.0;										// free element
	for ( int k = EIG_DIM-1-1; k >= 0; k -- )
	{
		vv[k] = 0.0;
		for ( int j = k+1; j < EIG_DIM; j ++ )
			vv[k] -= m[j][k] * vv[j];				// transpose
		vv[k] /= m[k][k];
	}
...
{	
	...
	vec2 texelSize = kernelSpread / textureSize(texture2D_aKernel, 0);

	vec3 a = texture(texture2D_aKernel, vScreenPos).xyz;
