...
uniform sampler2D edgeNearness;
...
{	
	...
	//FragColor.rgba = texture(pointCloud, vScreenPos);
	//return;
    ...
    float edgeDistance = texture(edgeNearness, vScreenPos).r;
	...
	for(int dX = -usedRadius; dX <= usedRadius; ++dX){
		for(int dY = -usedRadius; dY <= usedRadius; ++dY){
			...
			float edgeDist = texture(edgeNearness, coord).r;
